...
def make_episode_key(series_name, episode_number):
    ...
    safe_name = normalize_text(series_name).lower()

    # برای جلوگیری از فاصله‌های اضافه
    safe_name = re.sub(r"\s+", " ", safe_name).strip()

    return f"{safe_name}__{episode_number}"
...
def save_episode(series_name, episode_number, file_data):
    ...
    if len(existing) > 0:
        ...
        # اگر قبلاً همین فایل وجود نداشت
...
def show_join_step(chat_id):
    ...
    if sponsors:
        ...
        text = (
            "برای دریافت فایل باید اول در کانال‌های زیر عضو بشی 👇\n\n"
        )

        for i, sponsor in enumerate(sponsors, 1):

            text += f"{i}. {sponsor['title']}\n"
        ...
        send_message(
            chat_id,
            "برای دریافت فایل اول عضو کانال اصلی شو 👇",
            join_button()
        )
...
def continue_after_done(chat_id, user_id):
    ...
    if not pending:
        ...
        send_message(
            chat_id,
            "❌ لینک منقضی شده. دوباره لینک قسمت رو باز کن."
        )
    ...
    if data == "check_join":
        ...
        if not ok:
            ...
            if failed == "main":
                ...
                send_message(
                    chat_id,
                    f"❌ هنوز در کانال «{failed['title']}» عضو نشدی.",
                    sponsor_buttons()
                )
...
def handle_start(message):
    ...
    if not start_param.startswith("ep_"):
    ...
    if not ok:
        ...
        if failed == "main":
            ...
            send_message(
                chat_id,
                "برای دریافت این قسمت اول عضو کانال اصلی شو 👇",
                join_button()
            )
...
def handle_message(message):
    ...
    # اول ادمین
    if message.get("from", {}).get("id") == ADMIN_ID:
